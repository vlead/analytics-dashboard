...
<div id='dashboard' class="bottom">
  <svg id="fillgauge1" width="63%" height="250"></svg>
</div>
</div>
...
function dashboard(id, fData) {
  ...
  fData.forEach(function(d) {
    ...
    d.total = parseInt(d.freq.generic + d.freq.experiment + d.freq.lab);
  ...
  var tF = ['lab', 'generic', 'experiment'].map(function(d) {
    return {
      type: d,
      freq: d3.sum(fData.map(function(t) {
          
          console.log(t.freq[d]+t.freq[d+1])
        return t.freq[d]+t.freq[d+1];
      }))
    };
  });
...
function secondpie(Selectedversions){
  $.ajax({ // Ajax call for URL
  method: 'GET',
  url: "http://feedback-analytics.vlabs.ac.in/vlabs/feedback/_search?size=10000", // Keep URL here
  type: 'json' // define type here
